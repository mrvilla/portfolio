import { createASTArrayExpression, createASTExpressionTag, } from '#parser/ast';
/**
 ...
 * ```
 */
export function transformComponentMetaToDefineMeta(params) {
...
function transformMetaCommentToBlockComment(comment) {
    const { data } = comment;
    return {
        type: 'Block',
        value: `*${data.replace(/\n/, '\n* ')}`,
...
function attributeToObjectExpressionProperty(attribute) {
...
function attributeValueToPropertyValue(value) {
    if (value === true) {
        return {
            type: 'Literal',
            value: true,
...
function transformTags(tags) {
    ...
    if (tags.value[0].type === 'ExpressionTag' && tags.value[0].expression.type === 'Literal') {
        ...
        // NOTE: This is the only case so far where
        // we need to transform tags from singular string literal to an array expression - to align with CSF3 format
        tags.value = createASTExpressionTag(createASTArrayExpression([
            {
                type: 'Literal',
